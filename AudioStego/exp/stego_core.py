...
class StegoUtils:
    @staticmethod
    def compress_image(file_path, target_size, jpeg_quality=45):
        if Image is None:
        ...
        try:
            ...
            # Nén thẳng tay ở mức quality=45 (Tuyệt đối không dùng vòng lặp ép 10KB)
            output = io.BytesIO()
    ...
    def prepare_payload(file_path, target_size=256):
        ...
        if ext in image_exts:
            # Truyền quality=45 vào đây để đạt mức Rate 3.80 vật lý chuẩn bài báo
            return StegoUtils.compress_image(file_path, target_size, jpeg_quality=20) 
        else:
            with open(file_path, 'rb') as f:
                return f.read()
...
class StegoAlarood:
    @staticmethod
    def encode(cover_path: str, payload_bytes: bytes, output_path: str,
    ...
    def decode(stego_path: str) -> dict:
        try:
            ...
            return {"status": "error",
                    "message": "alarood_stego.py not found."}
...
class StegoPhase:
    ...
    def encode(cover_path, payload_bytes, output_path):
        try:
            ...
            if msg_len > capacity:
                return {"status": "error", "message": f"Oversize: need {msg_len} bits, capacity {capacity}"}
